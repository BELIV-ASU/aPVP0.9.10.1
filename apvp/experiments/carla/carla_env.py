...
class SteeringWheelController:
    ...
    def process_input(self, speed_kmh):
        ...
        if is_windows:
            ...
            #logger.info(f"Raw throttle: {raw_throttle}, raw brake: {raw_brake}")
            #logger.info(f"Processed throttle: {throttle}, brake: {brake}")
...
class HumanInTheLoopCARLAEnv(ContinuousBenchmarkEnvWrapper):
    def __init__(self, external_config=None):
        ...
        #super().__init__(external_config)
        self.fig = None
        self.ax = None
        self.ims = None
        self.raw_obs = None
        self.human_data = defaultdict(list)
    ...
    def step(self, action):
        ...
        self.human_data["steering_human"].append(human_action[0])
        self.human_data["throttle_human"].append(human_action[1])
    ...
    def postprocess_obs(self, raw_obs, in_reset=False):
        if not hasattr(self, 'fig') or self.fig is None:
            self.fig, self.ax = plt.subplots(3, 3, figsize=(15, 15))
            self.ims = []
            for i in range(9):
                im = self.ax[i // 3, i % 3].imshow(np.zeros((84, 84, 3)), cmap='gray')
                self.ax[i // 3, i % 3].set_title(f"Channel {i+1}")
                self.ax[i // 3, i % 3].axis('off')
                self.ims.append(im)
            plt.ion()
            plt.show()

        if "birdview" in raw_obs:
            ...
            #     for j in range(3):
            #         rgb_image[..., j] = self.raw_obs["birdview"][..., (i * 3 + j) % 9]
            #     self.ims[i].set_data(rgb_image)
