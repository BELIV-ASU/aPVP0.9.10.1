...
class SimpleCarlaEnv(BaseCarlaEnv):
    ...
    def compute_reward(self) -> Tuple[float, Dict]:
        ...
        if speed/self._max_speed_kmh < 1.2:
            speed_reward = 0.5 * (min(speed, self._max_speed_kmh) / self._max_speed_kmh) * correct_direction_factor
        else:
            speed_reward = 0.1 * (self._max_speed_kmh - speed) / self._max_speed_kmh * correct_direction_factor
        ...
        #print("distance_to_go: %f, target_distance: %f" % (distance_to_go, target_distance))
        if self._last_distance is None:
            driving_reward = 0
        else:
            driving_reward = 0.1 * (self._last_distance - cur_distance) * correct_direction_factor * lateral_factor
