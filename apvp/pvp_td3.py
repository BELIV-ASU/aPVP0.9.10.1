...
class PVPTD3(TD3):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        similarity_threshold_start = 1 
        similarity_threshold_end = 0.1   
        total_steps = 20000     

        for step in range(gradient_steps):
            self._n_updates += 1

            similarity_threshold = max(similarity_threshold_end, similarity_threshold_start - (similarity_threshold_start - similarity_threshold_end) * (self._n_updates/ total_steps))
            # Logging similarity_threshold
            self.logger.record("train/similarity_threshold", similarity_threshold)
            ...
            action_similaritys = F.mse_loss(current_expert_actions, replay_data.actions_novice, reduction='none')
            action_similaritys = action_similaritys[:,0] + action_similaritys[:,1]
            action_similaritys = action_similaritys.view(-1, 1)
            ...
            #action_similarity = action_similarity.sum(dim=1, keepdim=True)
            #action_similarity = action_similarity / action_similarity.max()
            # Compute critic loss
            
            critic_loss = []
            for (current_action, next_action, current_q_behavior, current_q_novice, action_similarity) in zip(replay_data.actions_novice, next_actions, current_q_behavior_values, current_q_novice_values, action_similaritys):
                ...
                #if self._n_updates <30000:
                l += th.mean(replay_data.interventions * self.cql_coefficient *(F.mse_loss(current_q_behavior, F.mse_loss(replay_data.actions_novice, replay_data.actions_behavior))))
                l += th.mean(replay_data.interventions * self.cql_coefficient *(F.mse_loss(current_q_novice, -F.mse_loss(replay_data.actions_novice, replay_data.actions_behavior))))

                if action_similarity > similarity_threshold:
                    l += self.cql_coefficient *(F.mse_loss(current_q_behavior, lumda* action_similarity))
                    l += self.cql_coefficient *(F.mse_loss(current_q_novice, - lumda * action_similarity))
                    # l += th.mean(
                ...
                #     (F.mse_loss(current_q_behavior, 0.1 * action_similaritys))
                # )
                # l += th.mean(
                #     #replay_data.interventions * 
                #     self.cql_coefficient *
                #     (F.mse_loss(current_q_novice, -lumda * action_similaritys))
                
                    # print('similarity loss: %f', action_similarity)
                #else:
                    # smoothness_loss = F.mse_loss(current_action, next_action)
                    # l += F.mse_loss(current_q_novice,smoothness_loss)
                    #print('smothness loss: %f', smoothness_loss)
