    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for step in range(gradient_steps):
            ...
            for (current_q_behavior, current_q_novice) in zip(current_q_behavior_values, current_q_novice_values):
                ...
                # original loss function. use fixed loss,1, to update 
                l += th.mean(
                ...
                #change the loss from 1 to the diffferences between the novice action and human actions
                # mse_per_element = F.mse_loss(replay_data.actions_novice, replay_data.actions_behavior,reduction='none')
                # mse_per_sample = mse_per_element.mean(dim=1, keepdim=True)

                # l += th.mean(
                #     replay_data.interventions * self.cql_coefficient *
                #     (F.mse_loss(current_q_behavior, self.q_value_bound * mse_per_sample))
                #     )
                # l += th.mean(replay_data.interventions * self.cql_coefficient *
                #     (F.mse_loss(current_q_novice, -self.q_value_bound *mse_per_sample))
                #     )
