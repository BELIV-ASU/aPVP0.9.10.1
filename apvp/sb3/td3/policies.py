from typing import Tuple,Any, Dict, List, Optional, Type, Union
import numpy as np
import gym
import torch as th
...
class TD3Policy(BasePolicy):
    ...
    def predict(self, observation, state: Optional[Tuple[np.ndarray, ...]] = None, episode_start: Optional[np.ndarray] = None, deterministic = False):
        # Note: 
        actions, state = super().predict(observation, state, episode_start, deterministic)
        if self.expert:
            observation_tensor, _ = self.obs_to_tensor(observation)
            with th.no_grad():
                output_au = self.expert(observation_tensor).cpu().numpy()
        return actions, state, output_au
